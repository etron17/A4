...
def s_t(t: float, A: float, B: float, C: float, mu: float,
        shift: float) -> float:
    """
...
def getFit(data: List[float], A: float, B: float, C: float, mu: float,
    ...
    for i in range(len(data)):
        mse += (s_t(i + 1, A, B, C, mu, shift) - data[i]) ** 2
    return mse / len(data)
...
def setParameter(data: List[float]) -> Tuple[
    ...
    :param data: Data to check MSE on
    :return: Tuple with List of MSE on the way to find MSE < 1 and A. B, C,
    mu and shift
    """
    mse: List[float] = []
    step = 10
            ...
            for C in range(10, 201, step):
                ...
                for mu in range(0, 201, step):
                    ...
                    for shift in range(0, 151, step):
                        an_mse = getFit(data, A / 100, B / 100, C / 100,
                                        mu / 100, shift / 100)
                        mse.append(an_mse)
                        if an_mse < 0.1:
                            found = True
...
def plot_data(data: List[float],
    ...
    model = []
    for i in range(120):
        model.append(s_t(i + 1, A, B, C, mu, shift))
